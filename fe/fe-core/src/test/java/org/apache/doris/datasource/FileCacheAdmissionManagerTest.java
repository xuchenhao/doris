...
public class FileCacheAdmissionManagerTest {
    ...
    public void testEmptyUserIdentity() {
        ...
        boolean result = manager.isAllowedTableLevel("", "catalog", "database", "table", reason);
    ...
    public void testInvalidUserIdentity() {
        AtomicReference<String> reason = new AtomicReference<>();
        boolean result = manager.isAllowedTableLevel("123user", "catalog", "database", "table", reason);
        Assert.assertFalse(result);
    ...
    public void testDefaultAllow() {
        ...
        boolean result1 = manager.isAllowedTableLevel("user", "catalog", "database", "table", reason1);
        Assert.assertTrue(result1);
        Assert.assertEquals("default rule", reason1.get());

        Config.file_cache_admission_control_default_allow = false;
        AtomicReference<String> reason2 = new AtomicReference<>();
        boolean result2 = manager.isAllowedTableLevel("user", "catalog", "database", "table", reason2);
    ...
    public void testCommonRule() throws Exception {
        ...
        AtomicReference<String> reason1 = new AtomicReference<>();
        boolean result1 = manager.isAllowedTableLevel("user", "catalog_1", "database", "table", reason1);
        Assert.assertTrue(result1);
        Assert.assertEquals("common catalog-level whitelist rule", reason1.get());

        AtomicReference<String> reason2 = new AtomicReference<>();
        boolean result2 = manager.isAllowedTableLevel("user", "catalog_2", "database_1", "table", reason2);
        ...
        boolean result3 = manager.isAllowedTableLevel("user", "catalog_3", "database_2", "table_1", reason3);
    ...
    public void testUserRule() throws Exception {
        ...
        AtomicReference<String> reason1 = new AtomicReference<>();
        boolean result1 = manager.isAllowedTableLevel("user_1", "catalog_4", "database", "table", reason1);
        Assert.assertTrue(result1);
        Assert.assertEquals("user catalog-level whitelist rule", reason1.get());
        AtomicReference<String> reason2 = new AtomicReference<>();
        boolean result2 = manager.isAllowedTableLevel("user_2", "catalog_4", "database", "table", reason2);
        Assert.assertFalse(result2);
        Assert.assertEquals("default rule", reason2.get());

        AtomicReference<String> reason3 = new AtomicReference<>();
        boolean result3 = manager.isAllowedTableLevel("user_1", "catalog_5", "database_4", "table", reason3);
        Assert.assertTrue(result3);
        Assert.assertEquals("user database-level whitelist rule", reason3.get());
        AtomicReference<String> reason4 = new AtomicReference<>();
        boolean result4 = manager.isAllowedTableLevel("user_2", "catalog_5", "database_4", "table", reason4);
        Assert.assertFalse(result4);
        Assert.assertEquals("default rule", reason4.get());

        AtomicReference<String> reason5 = new AtomicReference<>();
        boolean result5 = manager.isAllowedTableLevel("user_1", "catalog_6", "database_5", "table_4", reason5);
        Assert.assertTrue(result5);
        Assert.assertEquals("user table-level whitelist rule", reason5.get());
        AtomicReference<String> reason6 = new AtomicReference<>();
        boolean result6 = manager.isAllowedTableLevel("user_2", "catalog_6", "database_5", "table_4", reason6);
    ...
    public void testRuleLevelPriority() throws Exception {
        ...
        AtomicReference<String> reason1 = new AtomicReference<>();
        boolean result1 = manager.isAllowedTableLevel("user_3", "catalog", "database", "table", reason1);
        Assert.assertTrue(result1);
        ...
        AtomicReference<String> reason2 = new AtomicReference<>();
        boolean result2 = manager.isAllowedTableLevel("user_3", "catalog", "database", "table", reason2);
        Assert.assertTrue(result2);
        ...
        AtomicReference<String> reason3 = new AtomicReference<>();
        boolean result3 = manager.isAllowedTableLevel("user_3", "catalog", "database", "table", reason3);
        Assert.assertTrue(result3);
        Assert.assertEquals("user database-level whitelist rule", reason3.get());
        ...
        AtomicReference<String> reason4 = new AtomicReference<>();
        boolean result4 = manager.isAllowedTableLevel("user_3", "catalog", "database", "table", reason4);
    ...
    public void testRuleTypePriority() throws Exception {
        ...
        AtomicReference<String> reason1 = new AtomicReference<>();
        boolean result1 = manager.isAllowedTableLevel("user_4", "catalog", "database", "table", reason1);
        Assert.assertFalse(result1);
        ...
        AtomicReference<String> reason2 = new AtomicReference<>();
        boolean result2 = manager.isAllowedTableLevel("user_4", "catalog", "database", "table", reason2);
        Assert.assertFalse(result2);
        ...
        AtomicReference<String> reason3 = new AtomicReference<>();
        boolean result3 = manager.isAllowedTableLevel("user_4", "catalog", "database", "table", reason3);
        Assert.assertFalse(result3);
        ...
        AtomicReference<String> reason4 = new AtomicReference<>();
        boolean result4 = manager.isAllowedTableLevel("user_4", "catalog", "database", "table", reason4);
        Assert.assertFalse(result4);
    ...
    public void testNestedRulePriorities() throws Exception {
        ...
        AtomicReference<String> reason1 = new AtomicReference<>();
        boolean result1 = manager.isAllowedTableLevel("user_5", "catalog", "database", "table", reason1);
        Assert.assertTrue(result1);
        Assert.assertEquals("user database-level whitelist rule", reason1.get());
        AtomicReference<String> reason2 = new AtomicReference<>();
        boolean result2 = manager.isAllowedTableLevel("user_5", "catalog", "otherDatabase", "table", reason2);
        Assert.assertFalse(result2);
        Assert.assertEquals("user catalog-level blacklist rule", reason2.get());

        AtomicReference<String> reason3 = new AtomicReference<>();
        boolean result3 = manager.isAllowedTableLevel("user_6", "catalog", "database", "table", reason3);
        Assert.assertFalse(result3);
        Assert.assertEquals("user database-level blacklist rule", reason3.get());
        AtomicReference<String> reason4 = new AtomicReference<>();
        boolean result4 = manager.isAllowedTableLevel("user_6", "catalog", "otherDatabase", "table", reason4);
        Assert.assertTrue(result4);
        Assert.assertEquals("user catalog-level whitelist rule", reason4.get());

        AtomicReference<String> reason5 = new AtomicReference<>();
        boolean result5 = manager.isAllowedTableLevel("user_7", "catalog", "database", "table", reason5);
        Assert.assertTrue(result5);
        Assert.assertEquals("user table-level whitelist rule", reason5.get());
        AtomicReference<String> reason6 = new AtomicReference<>();
        boolean result6 = manager.isAllowedTableLevel("user_7", "catalog", "database", "otherTable", reason6);
        Assert.assertFalse(result6);
        Assert.assertEquals("user database-level blacklist rule", reason6.get());

        AtomicReference<String> reason7 = new AtomicReference<>();
        boolean result7 = manager.isAllowedTableLevel("user_8", "catalog", "database", "table", reason7);
        ...
        boolean result8 = manager.isAllowedTableLevel("user_8", "catalog", "database", "otherTable", reason8);
